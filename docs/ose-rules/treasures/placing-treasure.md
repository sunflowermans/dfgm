...
Monster listings note which treasure type is present in the creature’s lair and which it may carry on its person. See [Treasure Types](/docs/ose-rules/treasures/treasure-types/) for details on how to roll treasure according to a treasure type letter code.
...
The referee may wish to choose treasures appropriate to the monsters and the PC party. As treasure is the primary source of XP (see [Awarding XP](/docs/ose-rules/running-adventures/awarding-xp/)), the referee should carefully consider treasure placement. The following procedure is recommended:
...
The treasure tables (see [Treasure Types](/docs/ose-rules/treasures/treasure-types/)) describe methods for generating hoards of coins, gems, jewelry, and magic items.
...
For each unusual item placed, the referee should note its value (in gold pieces) and (if using the optional rules for [Encumbrance](/docs/ose-rules/adventures/time-weight-movement/#encumbrance)) its weight.
