...
Instead of an initiative roll per side, a roll may be made for each individual involved in a battle, modified by DEX (see [Dexterity](/docs/ose-rules/characters/ability-scores/#dexterity-dex) under [Ability Scores](/docs/ose-rules/characters/ability-scores/)). The referee may determine an initiative modifier for very fast or slow monsters instead of applying a DEX modifier.

## Monster Morale (Optional Rule)

See [Morale (Optional Rule)](/docs/ose-rules/adventures/morale/).
...
Possible when opponents are more than 5’ from one another (see [Attacking](/docs/ose-rules/adventures/combat/#attacking)).
...
Possible when opponents are 5’ or less from each other (see [Attacking](/docs/ose-rules/adventures/combat/#attacking)).
...
3. **Determine hit AC:** Look up the result in the attack matrix row for the attacker’s THAC0 (see [Combat Tables](/docs/ose-rules/adventures/combat-tables/)). This column is the AC score that the attack hits.
...
Groups using the optional rule for Ascending AC (see [Game Statistics](/docs/ose-rules/characters/game-statistics/)) should use the following attack procedure instead.
