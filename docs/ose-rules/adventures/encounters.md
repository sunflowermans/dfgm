...
(See [Combat](/docs/ose-rules/adventures/combat/) for complete details.)
...
**Charisma:** If one specific character attempts to speak with the monsters, that character’s NPC reactions modifier due to CHA (see [Ability Scores](/docs/ose-rules/characters/ability-scores/)) is used to modify the monster reaction roll.
...
- **Combat:** If one side attacks, casts spells, or makes tactical movements, begin tracking time in rounds, following the combat procedure (see [Combat](/docs/ose-rules/adventures/combat/)).
- **Evasion:** If one side decides to flee, the other may choose to pursue (see [Evasion](/docs/ose-rules/adventures/encounters/#evasion)).
- **Parley:** PCs may attempt to communicate with monsters. Opening an encounter in this way can influence the monsters’ behavior (see [Monster Actions](/docs/ose-rules/adventures/encounters/#monster-actions)). If both sides decide to talk, the negotiation may be role-played.
...
**Pursuit:** If the opposing side gives chase, the chance of the evasion succeeding depends on the explored environment. See [Evasion and Pursuit](/docs/ose-rules/adventures/evasion-and-pursuit/).
